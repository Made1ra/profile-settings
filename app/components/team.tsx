...
export default function Team() {
  ...
  function sortByTeamName() {
    ...
    teamNameSortOrder === "asc"
      ? dispatch(sortByTeamNameAsc())
      : dispatch(sortByTeamNameDesc());
  }

  function sortByMembers() {
    setMembersSortOrder((previous) => (previous === "asc" ? "desc" : "asc"));
    membersSortOrder === "asc"
      ? dispatch(sortByMembersAsc())
      : dispatch(sortByMembersDesc());
  }

  function sortByCreated() {
    setCreatedSortOrder((previous) => (previous === "asc" ? "desc" : "asc"));
    createdSortOrder === "asc"
      ? dispatch(sortByCreatedAsc())
      : dispatch(sortByCreatedDesc());
  }

  function sortByTeamMember(id: string) {
    setTeamMemberSortOrder((previous) => (previous === "asc" ? "desc" : "asc"));
    teamMemberSortOrder === "asc"
      ? dispatch(sortByTeamMemberAsc({ id }))
      : dispatch(sortByTeamMemberDesc({ id }));
  }
  ...
  function handleSwitch(id: string) {
    dispatch(
      addMember({
        id,
        email: "alla.abrosimova@gmail.com",
        accounts: 5,
        name: "Alla Abrosimova",
        isPending: true,
      })
    );
  }
            ...
            <tr>
              ...
              <th className="p-4 text-gray-500 max-sm:p-2">
                <div
                  ...
                  className="flex text-nowrap cursor-pointer"
                >
                  Team name
                  <ChevronUpDown />
                </div>
            ...
            {teams.map((team: Team) => (
              <Fragment key={team.id}>
                <tr className="bg-blue-50 my-2">
                  ...
                  >
                    <div
                      className="cursor-pointer rounded flex items-center justify-center w-10 h-10
                                        hover:bg-blue-200"
                    >
                  ...
                  <td className="p-4 max-sm:p-2">
                    <div className="flex items-center justify-start text-nowrap">
                      ...
                      <Badge
                        role={
                          team.teamOwner === "alla.abrosimova@gmail.com"
                            ? "Owner"
                            : "Member"
                        }
                      />
                  ...
                  <td
                    ...
                    ref={(el) => {
                      dropdownRefs.current[team.id] = el;
                    }}
                  >
                    <EllipsisVertical />
